(function () {
  ...
  */
  ...
  const registry = {};
  const register$3 = namespace => (key, stream) => {
  ...
  const getStream = (namespace, key) => {
    if (hasPath$1([namespace, key], registry)) {
      return path$1([namespace, key], registry);
    }

    throw new ReferenceError(`No stream at path '${[namespace, key]}'`);
  };

  const ns$2 = 'AudioPlayer';
  const register$2 = register$3(ns$2);
  function AudioPlayer(id) {
    ...
    audioEvts.forEach(evtName => {
      register$2(evtName, lib.stream());
      audio.addEventListener(evtName, getStream(ns$2, evtName));
    });
    lib.on(e => {
      audio.src = URL.createObjectURL(e.target.files[0]);
      audio.classList.remove('no-show');
    }, getStream('FileUpload', 'uploaded'));
    return getNamespace(ns$2);
  }

  const ns$1 = 'Controls';
  const register$1 = register$3(ns$1);
  ...
  function Controls(id) {
    const startStream = register$1('startMarked', lib.stream());
    const endStream = register$1('endMarked', lib.stream());
    ...
    const audio = getNamespace('AudioPlayer');
    lib.on(e => {
  ...
  function Logger(id) {
    ...
    lib.on(e => {
      log(logStr(e.type, e));
    }, getStream('Controls', 'startMarked'));
    lib.on(e => {
      log(logStr(e.type, e));
    }, getStream('Controls', 'endMarked'));
  }
